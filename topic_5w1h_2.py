...
def main():
    ...
    if "error" not in result:
        ...
        print("\n=== 詳細節點示例（When） ===")
        for node in analysis['detailed_nodes']['when_nodes']:
            print(f"- {node['label']}: {node['description']}")
...
if __name__ == "__main__":
    ...
    # 方法2: 分析並上傳到Supabase（單一關鍵字）
    demo_with_custom_requirements()
    
    # 方法3: 批次分析並上傳（多個關鍵字）
    # keywords_to_analyze = ["大罷免", "其他關鍵字1", "其他關鍵字2"]
    # batch_results = batch_analyze_and_upload(keywords_to_analyze)