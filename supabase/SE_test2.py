...
key_map = {
    ...
    "source": "sourcecle_media",
    "Image": "image",
    "sourceorg_media": "sourcecle_media",
}

# sourcecle_media = "BBC"  # 來源媒體名稱
image_name = None  # 圖片欄位名稱
...
def transform_single(obj):
    if isinstance(obj, dict):
        ...
        if len(new_obj["content"]) > 255:
            new_obj["content"] = new_obj["content"][:255]
        
        new_obj["sourcecle_id"] = generate_uuid(obj.get("sourcecle_id", ""))
        # new_obj["sourcecle_media"] = sourcecle_media  # 新增來源媒體名稱
        # new_obj["image"] = image_name  # 新增圖片欄位名稱
        return new_obj
