...
def get_current_max_id():
    try:
        res = supabase.table("crawler_news").select("sourceorg_id").order("sourceorg_id", desc=True).limit(1).execute()
        if res.data and len(res.data) > 0:
            return res.data[0]["sourceorg_id"]
        else:
            return 0
...
def crawl_news():
    ...
    # === 處理轉換 ===
    converted_data = transform_all(raw_data, current_max_id)
