...
【現有分支新聞】
{chr(1000).join(branch_summaries)}
{print(chr(1000).join(branch_summaries))}
...
"""
    ...
    def fetch_unbranched_news_for_topic(self, topic_id):
        """獲取專題中尚未分入任何分支的新聞
        ...
        """
        try:
            print(f"  查找主題 {topic_id} 中未分入分支的新聞...")
            ...
            all_story_ids = set(item['story_id'] for item in topic_news_response.data)
            print(f"  → 主題共有 {len(all_story_ids)} 則新聞")
            ...
            if not branch_response.data:
                print(f"  → 該主題尚無分支，所有新聞都未分支")
                return list(all_story_ids)
            
            branched_story_ids = set()
            for branch in branch_response.data:
                branch_id = branch.get('topic_branch_id')
                news_map_response = self.supabase.table('topic_branch_news_map').select(
                    'story_id'
                ...
                if news_map_response.data:
                    branched_story_ids.update(item['story_id'] for item in news_map_response.data)
            
            print(f"  → 已分支新聞: {len(branched_story_ids)} 則")
            
            # 3. 找出差異（未分支的新聞）
            unbranched_story_ids = all_story_ids - branched_story_ids
            print(f"  → 未分支新聞: {len(unbranched_story_ids)} 則")
            
            return list(unbranched_story_ids)
            
        except Exception as e:
            print(f"  ✗ 獲取未分支新聞時發生錯誤: {e}")
            return []
    
    def process_unbranched_news_for_topic(self, topic_id, save_to_db=True):
        """處理單個主題中未分支的新聞
        ...
        """
        print(f"\n{'='*80}")
        print(f"📋 處理主題 {topic_id} 的未分支新聞")
        print(f"{'='*80}")
        
        # 1. 獲取未分支的新聞
        unbranched_story_ids = self.fetch_unbranched_news_for_topic(topic_id)
        
        if not unbranched_story_ids:
            print("✓ 該主題沒有未分支的新聞")
            return None
        
        print(f"\n發現 {len(unbranched_story_ids)} 則未分支新聞，開始處理...")
        ...
        for branch in existing_branches:
            if self.OTHER_BRANCH_TITLE in branch['title']:
                other_branch = branch
                print(f"  → 找到現有「其他新聞」分支: {branch['news_count']} 則新聞")
            else:
                normal_branches.append(branch)
        
        print(f"  → 普通分支數量: {len(normal_branches)}")
        
        # 4. 獲取未分支新聞的內容
        unbranched_news_items = self.fetch_news_from_supabase(unbranched_story_ids)
        if not unbranched_news_items:
            print("✗ 無法獲取未分支新聞內容")
            return None
        
        # 5. 嘗試將每則未分支新聞加入現有分支
        branch_additions = {}  # {branch_id: [news_items]}
        unmatched_news = []
        
        print(f"\n開始比對未分支新聞與現有 {len(normal_branches)} 個分支...")
        
        for i, news in enumerate(unbranched_news_items, 1):
            print(f"\n  處理新聞 {i}/{len(unbranched_news_items)}: {news['news_title'][:40]}...")
            matched = False
            
            for branch in normal_branches:
                # 獲取該分支的代表性新聞用於比對
                branch_news = self.fetch_news_from_supabase(branch['story_ids'][:3])
                
                if self.check_if_news_fits_branch(news, branch_news):
                    print(f"    ✓ 可加入分支「{branch['title']}」")
                    if branch['branch_id'] not in branch_additions:
                        branch_additions[branch['branch_id']] = []
                    branch_additions[branch['branch_id']].append(news)
                    matched = True
                    break
            
            if not matched:
                print(f"    ○ 無法加入任何現有分支，存入待處理")
                unmatched_news.append(news)
        
        # 6. 合併無法匹配的新聞與現有"其他新聞"分支
        other_news_items = unmatched_news.copy()
        
        if other_branch:
            print(f"\n合併到現有「其他新聞」分支...")
            other_branch_news = self.fetch_news_from_supabase(other_branch['story_ids'])
            other_news_items.extend(other_branch_news)
            print(f"  現有其他新聞: {len(other_branch_news)} 則")
            print(f"  新增未匹配: {len(unmatched_news)} 則")
            print(f"  合併後共: {len(other_news_items)} 則")
        else:
            print(f"\n未找到現有「其他新聞」分支，準備創建新的")
            print(f"  未匹配新聞: {len(unmatched_news)} 則")
        
        # 7. 決定如何處理"其他新聞"
        new_branches_to_create = []
        updated_other_branch = None
        
        if len(other_news_items) >= self.MIN_BRANCH_SIZE:
            print(f"\n★ 「其他新聞」達 {len(other_news_items)} 則，超過最小規模 {self.MIN_BRANCH_SIZE}，嘗試重新分組...")
            ...
            for group in regrouped:
                if group.get('is_other', False):
                    # 仍然是"其他新聞"
                    updated_other_branch = group
                    print(f"  → 仍有 {group['news_count']} 則新聞不足規模，保留在「其他新聞」")
                else:
                    # 新生成的正式分支
                    new_branches_to_create.append(group)
                    print(f"  ✓ 生成新分支「{group['event_title']}」({group['news_count']} 則)")
        
        elif other_news_items:
            # 不足最小規模，繼續作為"其他新聞"
            print(f"\n→ 「其他新聞」僅 {len(other_news_items)} 則，不足最小規模，繼續保留")
            updated_other_branch = {
                'event_id': other_branch['branch_id'] if other_branch else str(uuid.uuid4()),
                'event_title': self.OTHER_BRANCH_TITLE,
                'event_summary': f'包含 {len(other_news_items)} 則待重新分類的新聞',
        ...
        if save_to_db:
            ...
            try:
                # 8.1 更新現有分支（加入新新聞）
                for branch_id, news_list in branch_additions.items():
                    print(f"\n更新分支 {branch_id}，加入 {len(news_list)} 則新聞...")
                    
                    # 準備 topic_branch_news_map 資料
                    map_data = [
                        {'topic_branch_id': branch_id, 'story_id': news['story_id']}
                        for news in news_list
                    ]
                    
                    # 寫入
                    if map_data:
                        response = self.supabase.table('topic_branch_news_map').insert(map_data).execute()
                        if response.data:
                            print(f"  ✓ 成功加入 {len(map_data)} 則新聞")
                        else:
                            print(f"  ✗ 寫入失敗")
                
                # 8.2 處理"其他新聞"分支
                if other_branch and updated_other_branch:
                    # 更新現有的"其他新聞"分支
                    print(f"\n更新現有「其他新聞」分支...")
                    
                    # 刪除舊的映射關係
                    self.supabase.table('topic_branch_news_map').delete().eq(
                        'topic_branch_id', other_branch['branch_id']
                    ).execute()
                    
                    # 寫入新的映射關係
                    map_data = [
                        {'topic_branch_id': updated_other_branch['event_id'], 'story_id': news['story_id']}
                        for news in updated_other_branch['news_items']
                    ]
                    
                    if map_data:
                        response = self.supabase.table('topic_branch_news_map').insert(map_data).execute()
                        if response.data:
                            print(f"  ✓ 更新完成，共 {len(map_data)} 則新聞")
                    
                    # 更新分支資訊
                    self.supabase.table('topic_branch').update({
                        'topic_branch_content': updated_other_branch['event_summary']
                    }).eq('topic_branch_id', other_branch['branch_id']).execute()
                
                elif updated_other_branch and not other_branch:
                    # 創建新的"其他新聞"分支
                    print(f"\n創建新的「其他新聞」分支...")
                    ...
                    branch_data = {
                        'topic_id': topic_id,
                        'topic_branch_id': updated_other_branch['event_id'],
                        'topic_branch_title': updated_other_branch['event_title'],
                        'topic_branch_content': updated_other_branch['event_summary']
                    }
                    response = self.supabase.table('topic_branch').insert([branch_data]).execute()
                    
                    if response.data:
                        print(f"  ✓ 分支創建成功")
                        
                        # 寫入 topic_branch_news_map
                        map_data = [
                            {'topic_branch_id': updated_other_branch['event_id'], 'story_id': news['story_id']}
                            for news in updated_other_branch['news_items']
                        ]
                        ...
                        if map_data:
                            response = self.supabase.table('topic_branch_news_map').insert(map_data).execute()
                            if response.data:
                                print(f"  ✓ 加入 {len(map_data)} 則新聞")
                
                # 8.3 創建新生成的分支
                if new_branches_to_create:
                    print(f"\n創建 {len(new_branches_to_create)} 個新分支...")
                    
                    for new_branch in new_branches_to_create:
                        print(f"\n  創建分支「{new_branch['event_title']}」...")
                        
                        # 寫入 topic_branch
                        branch_data = {
                            'topic_id': topic_id,
                            'topic_branch_id': new_branch['event_id'],
                            'topic_branch_title': new_branch['event_title'],
                            'topic_branch_content': new_branch['event_summary']
                        }
                        response = self.supabase.table('topic_branch').insert([branch_data]).execute()
                        ...
                        if response.data:
                            ...
                            # 寫入 topic_branch_news_map
                            map_data = [
                                {'topic_branch_id': new_branch['event_id'], 'story_id': news['story_id']}
                                for news in new_branch['news_items']
                            ]
                    ...
                    # 如果有新分支生成，且是從舊的"其他新聞"分支重組的，刪除舊分支
                    if other_branch:
                        print(f"\n  刪除舊的「其他新聞」分支...")
                        self.supabase.table('topic_branch_news_map').delete().eq(
                            'topic_branch_id', other_branch['branch_id']
        ...
        print("📊 處理摘要")
        print(f"{'='*80}")
        print(f"未分支新聞總數: {len(unbranched_story_ids)}")
        
        total_matched = sum(len(news_list) for news_list in branch_additions.values())
        print(f"成功加入現有分支: {total_matched} 則")
        
        for branch_id, news_list in branch_additions.items():
            branch_title = next((b['title'] for b in normal_branches if b['branch_id'] == branch_id), '未知分支')
            print(f"  - {branch_title}: +{len(news_list)} 則")
        
        print(f"無法加入現有分支: {len(unmatched_news)} 則")
        
        if updated_other_branch:
            print(f"「其他新聞」分支: {updated_other_branch['news_count']} 則")
        
        if new_branches_to_create:
            print(f"新生成分支: {len(new_branches_to_create)} 個")
            for branch in new_branches_to_create:
                print(f"  - {branch['event_title']}: {branch['news_count']} 則")
        
        return {
            'topic_id': topic_id,
            'unbranched_count': len(unbranched_story_ids),
            'matched_count': total_matched,
            'unmatched_count': len(unmatched_news),
            'new_branches': len(new_branches_to_create),
            'branch_additions': branch_additions,
            'new_branches_created': new_branches_to_create,
            'updated_other_branch': updated_other_branch
        }
...
def main():
    ...
    print("🚀 新聞事件分組器 - 啟動中...")
    print("=" * 80)
    print("📋 模式：處理未分支新聞（最小分支規模: 5 則新聞）")
    print("=" * 80)
    print("\n功能特點:")
    print("  ✓ 搜尋專題中尚未分入分支的新聞")
    print("  ✓ 先與既有分支進行比對，看能否分入")
    print("  ✓ 不行則存入「其他新聞」")
    print("  ✓ 若「其他新聞」超過 5 篇，重新生成新分支")
    print()
    ...
    # ⚠️ 重要設定
    save_to_db = False  # 設為 True 才會寫入資料庫
    
    print(f"💾 資料庫寫入: {'啟用' if save_to_db else '停用（僅預覽）'}")
    print()
    ...
    try:
        ...
        response = grouper.supabase.table('topic_news_map').select('topic_id').execute()
        ...
        # 取得唯一的 topic_id
        all_topic_ids = list(set(item['topic_id'] for item in response.data))
        print(f"✓ 找到 {len(all_topic_ids)} 個主題")
        
        # 2. 處理每個主題
        all_results = []
        
        for i, topic_id in enumerate(all_topic_ids, 1):
            print(f"\n{'='*80}")
            print(f"處理主題 {i}/{len(all_topic_ids)}: {topic_id}")
            print(f"{'='*80}")
            
            result = grouper.process_unbranched_news_for_topic(topic_id, save_to_db=save_to_db)
            
            if result:
                all_results.append(result)
            else:
                print(f"主題 {topic_id}: 無未分支新聞或處理完成")
        
        # 3. 輸出總體統計
        print("\n" + "=" * 80)
        print("📈 總體統計")
        print("=" * 80)
        print(f"處理的主題數: {len(all_topic_ids)}")
        print(f"有未分支新聞的主題: {len(all_results)}")
        
        if all_results:
            total_unbranched = sum(r['unbranched_count'] for r in all_results)
            total_matched = sum(r['matched_count'] for r in all_results)
            total_unmatched = sum(r['unmatched_count'] for r in all_results)
            total_new_branches = sum(r['new_branches'] for r in all_results)
            
            print(f"\n未分支新聞總計: {total_unbranched} 則")
            print(f"  ✓ 成功加入現有分支: {total_matched} 則")
            print(f"  ○ 無法加入現有分支: {total_unmatched} 則")
            print(f"  ★ 新生成分支數: {total_new_branches} 個")
            
            # 列出每個主題的詳細資訊
            print("\n詳細資訊:")
            for r in all_results:
                print(f"\n  主題 {r['topic_id']}:")
                print(f"    未分支: {r['unbranched_count']} 則")
                print(f"    已匹配: {r['matched_count']} 則")
                print(f"    未匹配: {r['unmatched_count']} 則")
                if r['new_branches'] > 0:
                    print(f"    新分支: {r['new_branches']} 個")
                    for branch in r.get('new_branches_created', []):
                        print(f"      - {branch['event_title']} ({branch['news_count']} 則)")
        ...
        if not save_to_db:
            print("\n📝 注意：目前為預覽模式，資料未寫入資料庫")
            print("若要寫入資料庫，請將 main() 中的 save_to_db 設為 True")
        else:
            print("\n✅ 資料已成功寫入資料庫")
