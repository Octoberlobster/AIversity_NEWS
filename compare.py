...
response = supabase.table("stories").select("story_id, story_url, crawl_date").order("crawl_date", desc=True).limit(300).execute()
...
with sync_playwright() as p:
    ...
    for story in stories:
        ...
        prefix_matches = [
            s for s in stories if s["story_url"][:88] == url_prefix
        ]
        ...
        for match_story in prefix_matches:
            ...
            crawl_date = match_story["crawl_date"]
