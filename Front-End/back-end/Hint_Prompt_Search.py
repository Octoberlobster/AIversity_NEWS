...
def genernate_hint_prompt():
    response = gemini_client.models.generate_content(
        model="gemini-2.0-flash",
        contents= f"使用者想要搜尋新聞，請你產生可能會用到的提示詞，生成 4 個最合適且能幫助使用者開始聊天的提示詞。",
...
def refresh_hint_prompt():
    genernate_hint_prompt.cache_clear()
...
# response = genernate_hint_prompt(tuple(option), article)
# print(response)
# option = ['科技', '健康']
# response = genernate_hint_prompt(tuple(option), article)
# print(response)
